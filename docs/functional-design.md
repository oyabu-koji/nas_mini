...
- 0.4.0 release orchestratorはoperator volume解除flagを持たず、manifest nonceとDocker labelで明示したdisposable DB volumeだけを受理する。current HEAD/manifest commitとhost contractのclean tracked stateを照合し、対象volumeをmountする全containerをDocker inventoryから検出してphaseごとの許可集合をcontainer ID/imageまで検証する。host検証後のpinned one-shotがowner-only volume markerを作り、全専用entrypointがmarkerを再検証する。002–007、008、worker-only drain、009 dry-run/apply、010 read-only preflightは固定serviceを`create --no-build --pull never`してactual project/image/Entrypoint/Cmd、image `Config.Env`とphase envのexact merge、mount/securityをinspect後に起動し、成功・失敗ともAPI/workerを停止したまま返す。同じmanifestはone-time claimにより再実行しない。既存008/009 host wrapperの自動再起動pathは使用しない。mutation開始後の失敗では停止確認後のDB `:ro` identity one-shotがactual last committed markerを返す。
- Actual contractは許可外env、追加persistent mount、privileged/device/host namespace、restart policy、想定外tmpfsを拒否する。create/inspect後かつstart直前に、対象exact created containerだけを許可して全volume consumerを再列挙する。旧008/009/010 host wrapperと旧Compose migratorは恒久fail closedとし、container DB pathを使う旧CLIもdisposable markerを要求する。
- 010 preflight-onlyはSQLite URI `mode=ro&immutable=1`の専用connectionとDB volume `:ro`を使い、既存WAL/SHMがあればopen前に拒否する。parent作成、journal mode変更、apply引数を持たず、main/WAL/SHMのcontentとmetadata不変をtestする。
- rollback drillは`/private/tmp/mediavault-operator-*`又は`/restore`のnonce marker付きdisposable rootだけでSQLite Backup API backup/restoreを行い、restore前にfailure main/WAL/SHMとbackup attestationをowner-only forensic artifactへ記録してからmarker/schema/integrity/FK/table aggregateを比較する。`MEDIA_ROOT`は同rootのstrict childに限定し、bounded inventoryでoriginal/pre-existing derivedを保護して明示したoperation-derived orphanだけを削除する。release/rollbackのimage IDとowner-only env artifactは別identityとし、rollback API/workerはimage IDを直接指定した停止containerとして再構成してexact actual env/command/mount/securityをinspect後、起動せず削除する。
