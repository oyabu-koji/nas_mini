...
- operator 0.4.0 safety implementationはoperator volume解除flagを持たず、manifest nonceとDocker disposable labelが一致するvolumeだけを扱う。host検証後のpinned marker initializerと全専用entrypointのowner-only marker guardを組み合わせる。002–007 offline CLI以外からstartup migrationを起動せず、DB volume consumerをlogical service名だけで判断しない。current HEADとhost contractのclean tracked state、全container inventory、phase allowlist、pinned actual container ID/project/image/env/Entrypoint/Cmd/mount/security、`create --no-build --pull never`を起動前に検証する。同じmanifestのone-time claim再実行、ambient Compose override、曖昧なcontainer state、失敗・interrupt・timeout時の停止未確認を拒否する。
- Actual container検証はpinned image `Config.Env`とphase envのexact merge以外、persistent mount exact set以外、privileged/device/host namespace/restart policy、exact size/optionsでないtmpfsを拒否し、create/inspect後とstart直前に全volume consumerを再列挙する。旧008/009/010 host wrapperと旧Compose migratorはfail closedで無効化し、container DB pathを使う旧CLIにもmarker guardを適用する。mutation開始後の失敗は停止確認後のDB `:ro` identity one-shotでactual last committed markerを取得し、phase履歴から推測しない。
- 010 preflight-onlyはwritable `connect()`を使わず、SQLite URI `mode=ro&immutable=1`とvolume `:ro`を使う。WAL/SHM存在時はopen前に拒否し、main/WAL/SHMの存在・type・size・digest・mtime/ctime不変をtestする。
- restore testは`/private/tmp/mediavault-operator-*`又はcontainer内`/restore`の固有label/nonce付きdisposable Docker volumeだけで行い、`MEDIA_ROOT`は同rootのstrict childに限定する。SQLite Backup API attestation、stale sidecar cleanup、marker/schema/integrity/FK/aggregate、bounded media inventoryを検証し、original/pre-existing derivedを削除・上書きせず、明示したoperation-derived orphanだけをcleanupする。
- Restore前にfailure main/WAL/SHM raw identity、取得可能なlogical identity、backup/attestation identityをowner-only forensic artifactへ記録する。008/009/010 commit後faultはservice errorの`restore_required=true`、orchestrator failureはlast committed version/restore要否/停止確認をtestする。
...
- operator migration test: fixed 002–007 success/transaction/commit fault、002–009 orchestration success/各phase failure/interrupt/unknown volume consumer/stop failure、010 main/WAL/SHM不変、002–010 faultごとのDB/sidecar/`MEDIA_ROOT` restore、release/rollback image/env mismatch、rollback停止container再構成を確認する。Docker drillはbuild/pullなしで固有disposable volumeだけを使う。
