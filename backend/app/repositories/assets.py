...
VERIFICATION_STATUS_FILE_VERIFIED = "file_verified"
...
def insert_verified_video_asset(
    conn: sqlite3.Connection,
    *,
    filename: str,
    original_path: str,
    ...
    cursor = conn.execute(
        """
        INSERT INTO assets (
            type, filename, original_path, size_bytes, server_sha256, taken_at,
            latitude, longitude, exif_json, is_log, transfer_status,
            verification_status, preview_status, review_status, delete_candidate_status
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            "video",
            ...
            1 if is_log else 0,
            TRANSFER_STATUS_UPLOADED,
            VERIFICATION_STATUS_FILE_VERIFIED,
            PREVIEW_STATUS_PREVIEW_GENERATING,
            REVIEW_STATUS_NOT_REVIEWED,
            DELETE_CANDIDATE_STATUS_NOT_CANDIDATE,
        ),
    ...
    if row is None:
        raise RuntimeError("verified asset could not be loaded")
    return dict(row)
